...
class Akatusbase_Akatusbase_RefundController extends Mage_Adminhtml_Controller_Action
{
    public function indexAction()
    {
        ...
        if ($orderId) {
            $paymentMethod =  new Akatusbase_Akatusbase_Model_Pagar();
            $url = $paymentMethod->getEstornoUrl();
            ...
            $responseArray = $paymentMethod->xml2array($response);
    ...
    private function getXML($order)
    {
        ...
        $apiKey = Mage::getStoreConfig('payment/akatusbase/api_key', $order->getStoreId());
        $email = Mage::getStoreConfig('payment/akatusbase/email_gateway', $order->getStoreId());
