...
require_once 'app/code/community/Akatusbase/Akatusbase/Model/Order.php';
...
$tokenNIP = Mage::getStoreConfig('payment/akatusbase/tokennip', $storeId);

if($tokenNIP == $tokenRecebido) {

    Mage::log('##### Token NIP correto, iniciando atualização de status...');
    Mage::log('## Status de transação recebido via NIP: ' . $statusAkatus);
    Mage::log('## ID do pedido localizado: ' . $order->getId());
    ...
    if ($newOrderState) {
        ...
        Mage::Log('##### Pedido ID ' . $order->getId() . ' atualizado do state ' . $currentOrderState . ' para ' . $newOrderState);
    }
}
...
function getNewOrderState($statusAkatus, $order)
{
    ...
    switch ($statusAkatus) {
        ...
        case StatusTransacaoAkatus::DEVOLVIDO:
            ...
            if (in_array($currentOrderState, $statusList)) {
                return Akatusbase_Akatusbase_Model_Order::STATE_REFUNDED;                    
            } else {
                return false;
            }
...
function updateOrder($order, $newOrderState) {
    ...
    } else if (podeEstornar($order) && $newOrderState === Akatusbase_Akatusbase_Model_Order::STATE_REFUNDED) {
